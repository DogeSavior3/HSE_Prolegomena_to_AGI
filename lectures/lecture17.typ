...
0) Для каждой точки выборки $X_i$ считаем расстояние до $k$-го ближайшего соседа $rho_k (x_i)$ и считаем объем шара $v_k (x_i)$ с центром в точке $x_i$ и радиусом, равным $rho_k (x_i)$. После чего мы считаем значимость наблюдения $x_i$ как $p_k (x_i) = k/(N v_k (x_i))$. После этого идет сортировка $x_i$ по убыванию их значимости, либо по возростанию объема $v_k (x_i)$. Это два достаточно времяемких действия, но существуют алгоритмы быстрого поиска этих статистик. 
